...
function categoryPoisson(){
    courses.forEach((food) => {
        ...
        if(!food.category.includes("Poisson")){
            // console.log(food);
            let noFish = document.querySelector(".main_container_card");
            noFish.className = "main_container_card noFish";
            document.querySelector(".noFish").style.display = "none";
            // console.log(noFish);
            // document.querySelector("main_container_card").style.display = "none";
        }
    });
    console.log(document.querySelectorAll(".main_container_card"));
}
// 1 fonction par tri (triPoisson / triVégé / triViande / triDessert) ? -> inconvénient: 1 bouton par fonction 
// 1 fonction qui reprend tous les tris cités au dessus -> inconvénient: paramètre (catégorie) pour la fonction ?
